...
const cssExtensionsRegex = /\.(html|css)$/;
...
export const resolve = async (specifier, context, defaultResolve) => {
  ...
  if (url) {
    ...
    if (cssExtensionsRegex.test(url.pathname)) {
      return {
...
export function getFormat(url, context, defaultGetFormat) {
  ...
  if (cssExtensionsRegex.test(url)) {
    return {
      format: 'module',
    };
  }
...
export const load = (url, context, defaultLoad) => {
  ...
  if (cssExtensionsRegex.test(new URL(url).pathname)) {
    return {
      format: 'module',
      source: `export default ${JSON.stringify(url.toString())}`,
    };
  }
